...
public class SubtractBed {
     ...
     *
     * @param i an ArrayList of TagNode representing the data to be subtracted
     * from
     * @param e an ArrayList of TagNode representing the data to be subtracted
     * by
     */
            ...
            if (ex.containsKey(chr)) {
                ...
                for (int i = 0; i < inTemp.size(); i++) {
					 ...
					 *     ====
                        ...
                        /**
                         * //New Approach. Use an array int[] keep = new
                         * int[inTemp.get(i).getLength()]; //String chr =
                         * inTemp.get(i).getChrom(); int bedStart =
                         * inTemp.get(i).getStart(); //int bedStop =
                         * inTemp.get(i).getStop(); for (int x = 0;x <
                         * results.size();x++){ int start =
                         * results.get(x).getHit().getStart(); int stop =
                         * results.get(x).getHit().getStop(); for (int y =
                         * start;y < stop;y++){
                         * if ((y-bedStart) >= 0 && (y-bedStart) < keep.length){
                         * keep[y-bedStart]++; } } }
                         *
                         * ArrayList<PileupNode2> pile = new
                         * ArrayList<PileupNode2>(); for (int x = 0;x <
                         * keep.length;x++){ if(keep[i] == 0){ PileupNode2 pnode
                         * = new PileupNode2(i+bedStart,0,chr); pile.add(pnode);
                         * } } output.addAll((new
                         * PileupToBedGraph(pile,1).getBedGraph()));
					*
                         */
                    }
     ...
     * @return a HashMap of String and ArrayList of TagNode where the key String
     * is the chromosome and the value ArrayList is all TagNode on that
     * chromosome
     */
